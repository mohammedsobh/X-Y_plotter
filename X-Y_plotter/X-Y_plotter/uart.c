...
// {
// 
// 	UBRRL = 0x67;                     //(uint8_t)(MyBaud);
// 	UBRRH = 0x00;                   //(uint8_t)(MyBaud>>8);
// 	
// 	UCSRB = ((1<<4) | (1<<3));     //Enable transmitter and Receiver 
// 	UCSRC = ((1<<2) | (1<<1));     //Character size is 8_bits
// 	
// }
// 
// 
...
// 	UDR = data;                   //transmits one character.
// }
// 
// char Recive_Char()
// {
// 	while (!(UCSRA & (1<<7)));   //waits until 'bin7' equal to 1, have unread data in the receive buffer
...
void UART_Init(uint32_t v_baudRate_u32)
{
	...
	UCSRC= UCSRC_SELECT | (1<<UCSZ1) | (1<<UCSZ0);   // Asynchronous mode 8-bit data and 1-stop bit
